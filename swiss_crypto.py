...
from Crypto.Hash import SHA256, SHA224, SHA384, SHA512, MD5, SHA1
...
def hash_sha224(data):
    hash_obj = SHA224.new(data.encode())
    return hash_obj.hexdigest()
...
def hash_sha384(data):
    hash_obj = SHA384.new(data.encode())
    return hash_obj.hexdigest()
...
def hash_data():
    ...
    if hash_type.get() == "MD5":
        result = hash_md5(data)
    elif hash_type.get() == "SHA-1":
        result = hash_sha1(data)
    elif hash_type.get() == "SHA-224":
        result = hash_sha224(data)
    elif hash_type.get() == "SHA-256":
        result = hash_sha256(data)
    elif hash_type.get() == "SHA-384":
        result = hash_sha384(data)
    elif hash_type.get() == "SHA-512":
        result = hash_sha512(data)
...
tk.Label(frame_encrypt, text="Select Cipher:").pack()
cipher_menu_encrypt = tk.OptionMenu(frame_encrypt, cipher_type, "AES", "DES", "ARC4")
cipher_type.set("Select Cipher")
...
tk.Label(frame_decrypt, text="Select Cipher:").pack()
cipher_menu_decrypt = tk.OptionMenu(frame_decrypt, cipher_type, "AES", "DES", "ARC4")
cipher_type.set("Select Cipher")
...
tk.Label(frame_hash, text="Select Hash Type:").pack()
hash_menu = tk.OptionMenu(frame_hash, hash_type, "MD5", "SHA-1", "SHA-224", "SHA-256", "SHA-384", "SHA-512")
